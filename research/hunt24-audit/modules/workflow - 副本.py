...
# 基础设施与核心科学组件导入
# from modules.db import AstroDBFacade, AssetManager
...
class AstroWorkflow:
    ...
    def __init__(self, db: AstroWorkflow):
        """初始化工作流引擎。
    ...
    def execute(
        ...
        self.logger.info(f"   对账文献: {category} | 资产装载策略: {params}")
        self.logger.info("=" * 80)
        ...
        # ----------------------------------------------------------------------
        self.logger.info(f"🔄 [Stage 1.75] 激活星团科学物理先验参数装载，执行方法: cluster.hydrate_from_db(strategy='{params}')...")
        hydrate_success = cluster.load_or_reconstruct_parameters(mode=params)
