...
class UnifiedMemberValidator:
    ...
    def _audit_physical_consistency(self, audit_matrix: pd.DataFrame) -> pd.DataFrame:
        """[策略分发器] 根据 VALIDATION_STRATEGY 路由到对应的物理验证算法。

        三种策略:
          - "chi2_upmask"       (默认): 卡方检验 + pyUPMASK CMD,         p ≥ 0.50
          - "chi2_cmd_residual" (实验A): 卡方检验 + 等龄线插值残差 CMD,  p ≥ 0.05
          - "weighted_penalty"  (实验B): 启发式加权惩罚分（修正后 backup 版本）
        """
        ...
        existing_req = [c for c in required_cols if c in audit_matrix.columns]
        nan_mask = audit_matrix[existing_req].isna().any(axis=1)
        if nan_mask.any():
            self.logger.warning(
                f"⚠️ [PhysAudit] 自动强制剔除 {nan_mask.sum()} 行包含关键字段 NaN 的样本。"
            )
            audit_matrix = audit_matrix[~nan_mask].copy()

        self.logger.info(
            f"🔍 [PhysAudit] 启动物理一致性审计。样本: {len(audit_matrix)}, "
            f"维度: {dim_mode}"
        )

        # ── 策略路由 ──
        strategy = getattr(cfg, "VALIDATION_STRATEGY", "chi2_upmask")
        self.logger.info(f"🔬 [PhysAudit] 验证策略: {strategy}")

        if strategy == "weighted_penalty":
            audit_matrix = self._phys_audit_weighted_penalty(
                audit_matrix, is_2d, is_physical_v
            )
            audit_matrix["pm_score"] = audit_matrix.get("pm_score", 0.0)
            self._log_phys_audit_stats(audit_matrix)
            return audit_matrix

        # ── chi2 系列策略：共享运动学 / 视差 / RV 卡方 ──
        audit_matrix["total_integrated_chi2"] = 0.0
        audit_matrix["total_dof"] = 0

        self._compute_kine_chi2(audit_matrix, is_2d, is_physical_v)
        self._compute_plx_chi2(audit_matrix, is_2d)
        self._compute_rv_chi2(audit_matrix)

        # ── CMD 维度 + 决策 ──
        if strategy == "chi2_cmd_residual":
            self._cmd_interpolator_residual(audit_matrix)
            audit_matrix = self._decide_chi2(audit_matrix, p_threshold=0.05)
        else:  # "chi2_upmask" (default)
            self._cmd_upmask(audit_matrix)
            audit_matrix = self._decide_chi2(audit_matrix, p_threshold=0.50)

        # ── 共享：向后兼容字段 ──
        dof_safe = audit_matrix["total_dof"].clip(lower=1)
        audit_matrix["weighted_penalty"] = audit_matrix["total_integrated_chi2"] / dof_safe
        ...
        try:
            if hasattr(self, "db") and self.db is not None:
                self.logger.info(
                    "💾 [PhysAudit] 检测到活跃 DuckDB 连接（落盘桩，当前为 no-op）。"
                )
        except Exception as db_err:
            self.logger.error(f"❌ [PhysAudit] DuckDB 落盘失败: {db_err}")
        return audit_matrix
    ...
    def _compute_kine_chi2(self, audit_matrix, is_2d, is_physical_v):
        """计算动力学卡方（3D 速度 或 2D 自行马氏距离）。"""
        if is_physical_v and all(c in audit_matrix.columns for c in ["u", "v", "w"]):
            ...
            kine = (u_res / self.get_param("U_ERROR")) ** 2 \
                 + (v_res / self.get_param("V_ERROR")) ** 2 \
                 + (w_res / self.get_param("W_ERROR")) ** 2
            dof = 3
        else:
        ...
        audit_matrix["kine_chi2"] = kine
        audit_matrix["total_integrated_chi2"] += kine
        audit_matrix["total_dof"] += dof

    def _compute_plx_chi2(self, audit_matrix, is_2d):
        """计算视差卡方 (DoF=1)。"""
        if is_2d:
            return
        plx_err = self.get_param("PLX_ERROR", 1.0)
        plx_chi2 = (audit_matrix["plx_residual"] / plx_err) ** 2
        audit_matrix["plx_chi2"] = plx_chi2
        audit_matrix["total_integrated_chi2"] += plx_chi2
        audit_matrix["total_dof"] += 1

    def _compute_rv_chi2(self, audit_matrix):
        """计算视向速度卡方 (DoF=1)，仅对有 RV 观测的行生效。"""
        if "rv" not in audit_matrix.columns:
            return
        rv_err = self.get_param("RV_ERROR", 5.0)
        rv_ref = self.get_param("RV_REF", 0.0)
        has_rv = audit_matrix["rv"].notna()
        if not has_rv.any():
            return
        rv_chi2 = ((audit_matrix.loc[has_rv, "rv"] - rv_ref) / rv_err) ** 2
        audit_matrix.loc[has_rv, "rv_chi2"] = rv_chi2
        audit_matrix.loc[has_rv, "total_integrated_chi2"] += rv_chi2.values
        audit_matrix.loc[has_rv, "total_dof"] += 1

    # =========================================================================
    # CMD 子策略
    # =========================================================================

    def _cmd_upmask(self, audit_matrix):
        """CMD 维度：pyUPMASK 聚类概率 → χ² (现有默认策略)。"""
        if not all(c in audit_matrix.columns for c in ["ra", "dec", "color", "mag"]):
            return
        ...
        valid_cmd = audit_matrix[upmask_cols].notna().all(axis=1)
        if not valid_cmd.any():
            return

        valid_df = audit_matrix[valid_cmd].copy()
        valid_idx = valid_df.index
        n_iter = int(self.get_param("UPMASK_ITERATIONS", 20))
        max_cl = int(self.get_param("UPMASK_MAX_CLUSTERS", 5))
        ...
        audit_matrix.loc[valid_idx, "upmask_prob"] = upmask_prob
        ...
        if "color_excess" in audit_matrix.columns and "color_excess_sigma" in audit_matrix.columns:
            outlier = np.abs(audit_matrix["color_excess"]) > (5.0 * audit_matrix["color_excess_sigma"])
            cmd_chi2[outlier[valid_cmd].values] += 1.0

        audit_matrix.loc[valid_idx, "cmd_residual"] = cmd_chi2
        audit_matrix.loc[valid_idx, "cmd_chi2"] = cmd_chi2

        # 仅 PG Only 星累加 CMD 卡方
        pg_only_valid = (~is_in_external) & valid_cmd
        if pg_only_valid.any():
            audit_matrix.loc[pg_only_valid, "total_integrated_chi2"] += audit_matrix.loc[pg_only_valid, "cmd_residual"]
            audit_matrix.loc[pg_only_valid, "total_dof"] += 1

    def _cmd_interpolator_residual(self, audit_matrix):
        """CMD 维度：等龄线插值残差 → χ² (实验策略 A)。

        使用 StarCluster 的 cmd_interpolator，与 backup 版等价但输出为 χ² 形式。
        修复了 backup 版 BP/RP 缺距离模数的 bug（已在 _setup_physical_constraints 中修正）。
        """
        if not all(c in audit_matrix.columns for c in ["color", "mag"]):
            return

        cmd_interp = self.cluster_obj.cmd_interpolator
        c_min, c_max = self.cluster_obj.cmd_color_bounds
        ...
        raw_res = audit_matrix["mag"].values - cmd_interp(audit_matrix["color"].values)
        # 非对称修正：联星方向（负残差）权重减半；超出色指数范围 ×1.5
        ...
        cmd_dev = self.get_param("CMD_DEV", 0.8)
        cmd_chi2 = (cmd_res / cmd_dev) ** 2  # (residual/σ)² → χ² 形式

        audit_matrix["cmd_residual"] = cmd_res
        audit_matrix["cmd_chi2"] = cmd_chi2
        audit_matrix["total_integrated_chi2"] += cmd_chi2
        audit_matrix["total_dof"] += 1  # DoF=1 for CMD interpolator
    ...
    def _decide_chi2(self, audit_matrix, p_threshold=0.50):
        """统一卡方假设检验决策。"""
        from scipy.stats import chi2

        dof_safe = audit_matrix["total_dof"].clip(lower=1)
        audit_matrix["global_cluster_probability"] = chi2.sf(
            audit_matrix["total_integrated_chi2"], dof_safe
        )
        audit_matrix["is_phys_consistent"] = (
            audit_matrix["global_cluster_probability"] >= p_threshold
        )

        self.logger.info(f"📊 [卡方检验报告] p 阈值 = {p_threshold}:")
        for dof in sorted(audit_matrix["total_dof"].unique()):
            dof_mask = audit_matrix["total_dof"] == dof
            if not dof_mask.any():
                continue
            chi2_cutoff = chi2.ppf(p_threshold, max(int(dof), 1))
            passed = (dof_mask & audit_matrix["is_phys_consistent"]).sum()
            total = dof_mask.sum()
            self.logger.info(
                f"  -> DoF={int(dof)}: χ²临界={chi2_cutoff:.3f}, "
                f"通过={passed}/{total} ({passed/total*100:.1f}%)"
            )
        return audit_matrix

    # =========================================================================
    # 实验策略 B：启发式加权惩罚分（修正 backup 版 bug）
    # =========================================================================

    def _phys_audit_weighted_penalty(self, audit_matrix, is_2d, is_physical_v):
        """启发式加权惩罚分验证（修正 backup 版）。

        与 backup 版的核心差异：
          - 运动学改为马氏距离（修复了 backup 未使用 pm_corr 的问题）
          - CMD 使用已修正距离模数的 StarCluster.cmd_interpolator
          - 其余维度与 backup 逻辑一致
        """
        penalties: dict = {}
        audit_matrix["cmd_residual"] = np.nan
        ...
        # ── A. 动力学 ──
        if is_physical_v and all(c in audit_matrix.columns for c in ["u", "v", "w"]):
            v_ref = self.get_param("UVW_REF", np.zeros(3))
            u_res = audit_matrix["u"] - v_ref[0]
            v_res = audit_matrix["v"] - v_ref[1]
            w_res = audit_matrix["w"] - v_ref[2]
            penalties["pm"] = np.sqrt(
                (u_res / self.get_param("U_ERROR", 2.5)) ** 2
                + (v_res / self.get_param("V_ERROR", 1.8)) ** 2
                + (w_res / self.get_param("W_ERROR", 1.2)) ** 2
            )
        else:
            pmra_disp = self.get_param("PMRA_DISPERSION", 3.0)
            pmdec_disp = self.get_param("PMDEC_DISPERSION", 3.0)
            penalties["pm"] = np.sqrt(
        ...
        # ── B. 视差 (仅 3D+) ──
        if not is_2d:
            plx_err = self.get_param("PLX_ERROR", 0.5)
        ...
        # ── D. CMD (使用已修正距离模数的 StarCluster 插值器) ──
        if self.cluster_obj.cmd_interpolator is not None \
           and all(c in audit_matrix.columns for c in ["color", "mag"]):
            ...
            out_mask = (audit_matrix["color"] < c_min) | (audit_matrix["color"] > c_max)
            cmd_res[out_mask] *= 1.5
            audit_matrix["cmd_residual"] = np.abs(cmd_res)
            cmd_dev = self.get_param("CMD_DEV", 0.8)
        ...
        kine_limit = self.get_param("KINE_SCORE_LIMIT", 2.0)
        kine_valid = audit_matrix["pm_score"] < kine_limit
        if not is_2d:
            kine_valid &= audit_matrix["plx_score"] < kine_limit
        if "rv" in audit_matrix.columns:
            kine_valid &= audit_matrix["rv"].isna() | (audit_matrix["rv_score"] < kine_limit)
        ...
        self.logger.info(
            ...
            f"通过={audit_matrix['is_phys_consistent'].sum()}/{len(audit_matrix)}"
        )
        return audit_matrix
