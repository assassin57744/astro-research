...
from modules.auditor import create_auditor, LiteratureAuditor
...
class UnifiedMemberValidator:
    ...
    def __init__(self, cluster: StarCluster, feature_space="5d", db_instance=None, cache_dir=None):
        ...
        # 🎯 CMD 插值网格已由 StarCluster 在构造阶段自动完成，无需在此重复构建

        # 文献审计器（懒初始化，首次使用时构建关键词网）
        self._lit_auditor = LiteratureAuditor(self.cluster_obj, self.cluster_id, self.logger)
    ...
    def _audit_physical_consistency(self, audit_matrix: pd.DataFrame) -> pd.DataFrame:
        """[策略分发器] 委托 auditor 模块执行物理验证。

        策略选择由 cfg.VALIDATION_STRATEGY 决定：
          - "chi2_upmask"       (默认): 卡方检验 + pyUPMASK CMD,         p ≥ 0.50
          - "chi2_cmd_residual" (实验A): 卡方检验 + 等龄线插值残差 CMD,  p ≥ 0.05
          - "weighted_penalty"  (实验B): 启发式加权惩罚分
        """
        if audit_matrix.empty:
            return audit_matrix
        ...
        # NaN 过滤 (auditor 基类提供)
        from modules.auditor import BasePhysicalAuditor
        ...
        auditor = create_auditor(strategy, self.cluster_obj, self.logger, self.cluster_id)
        ...
        # 委托策略执行
        if hasattr(auditor, "audit_with_dim"):
            audit_matrix = auditor.audit_with_dim(audit_matrix, dim_mode)
        else:
            audit_matrix = auditor.audit(audit_matrix)

        # 清理临时列
        if "id_str" in audit_matrix.columns:
            audit_matrix.drop(columns=["id_str"], inplace=True)

        auditor._log_stats(audit_matrix)
        return audit_matrix
    ...
    def _audit_literature_consensus(self, audit_matrix: pd.DataFrame) -> pd.DataFrame:
        """[委托] 文献共识审计，由 LiteratureAuditor 执行。"""
