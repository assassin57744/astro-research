...
def run_pleiades_verification():
    # --- 配置与路径 ---
    module_base = Path("D:/git/repo/Alapha/astro_research/modules/db/")
    dirs = {
        "warehouse": module_base / "warehouse",
    }
    db_path = dirs["warehouse"] / "astrodb_internal.db"
    view_name = "res_new"

    # --- 统计计数器 ---
    stats = {"total": 0, "confirmed": 0, "unconfirmed": 0, "errors": 0}
    ...
    try:
        ...
        for gaia_id in target_ids:
            ...
            try:
                ...
                if not star_info:
                    # 情况 2：结果为空，打印异常并继续
                    print(
                        f"{search_id:<35} | [!] Skipping    | SIMBAD returned no data"
                    )
                    stats["errors"] += 1
                    continue
                ...
                if melotte_pattern.search(ids_str) or m45_pattern.search(ids_str):
                    print(
                        f"{search_id:<35} | [√] Confirmed   | Member of Pleiades Cluster"
                    )
                    stats["confirmed"] += 1
                else:
                    print(
                        f"{search_id:<35} | [ ] Unconfirmed | Other identifiers found"
                    )
                    stats["unconfirmed"] += 1
        ...
        print("-" * 40)
        confirm_rate = (
            (stats["confirmed"] / stats["total"] * 100) if stats["total"] > 0 else 0
        )
    ...
    try:
        ...
        if result_table is not None and len(result_table) > 0:
            ...
            if "parent_name" in df.columns:
                df["parent_name"] = df["parent_name"].str.replace(
                    "NAME ", "", regex=False
                )
            ...
            # =========================================================
            df = df.drop_duplicates(subset=["child_name", "parent_name"]).reset_index(
                drop=True
            )
    ...
    try:
        ...
        if result_table is not None and len(result_table) > 0:
            ...
            if "child_name" in df.columns:
                df["child_name"] = df["child_name"].str.replace(
                    "NAME ", "", regex=False
                )
...
def query_simbad_parent_cluster(gaia_dr3_id) -> pd.DataFrame:
    ...
    try:
        print(
            f"正在向 SIMBAD 通过 [h_link] 检索母体结构，子源 ID: {full_child_name} ..."
        )
        # 3. 执行官方 TAP 查询
        result_table = Simbad.query_tap(query)
...
def generate_simbad_complete_review_report(gaia_dr3_id):
    ...
    try:
        print(
            f"正在依据 Schema 图谱全量抽取数据 [包含 membership 与 跨表 ref.bibcode]..."
        )
        ...
        if res_parents is not None and len(res_parents) > 0:
            ...
            df_right = pd.DataFrame(
                columns=["parent_name", "parent_type", "membership", "link_ref"]
            )
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 90)
    print(
        "                      SIMBAD 官方物理树扁平化科学审计报告                     "
    )
    print("=" * 90)
    print(f"审计目标 Gaia DR3 ID: {target_gaia_id} \n")
    print(
        "报告说明: 该表格包含了目标天体的所有别名（左侧）以及其在 SIMBAD 中被识别的所有母体结构信息（右侧），"
    )
    print(
        "包括母体名称、类型、成员关系以及相关文献引用。表格完全保留了 SIMBAD 中的原始数据，"
        "不进行任何形式的截断或归一化，适用于科学复核和多命名证认审计。"
    )
    print("-" * 90)
